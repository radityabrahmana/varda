/**
 * Builds the clause-library + past-feedback context injected into the AI review.
 * The pure builders are ported verbatim from Janus (janus-tools/agentActions.ts);
 * the queries run against MIKE's own tables so the moat (approved clauses +
 * C-level corrections) stays in Mike's DB.
 */
import { createServerSupabase } from "./supabase";

type Db = ReturnType<typeof createServerSupabase>;

type ClauseContextRow = {
  title: string;
  wording: string;
  source_type: string;
...
type MissedClauseRow = {
  highlight_text: string | null;
  suggested_category: string | null;
  user_note: string | null;
};

function buildClauseLibraryContext(clauses: ClauseContextRow[]): string {
...
function buildPastFeedbackContext(input: {
  clientName: string;
  docType: string;
